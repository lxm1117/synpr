...
#define Ra Ra_kv
 double Ra = 0.02;
#define ntau_kv _thread1data[0]
#define ntau _thread[_gth]._pval[0]
#define ninf_kv _thread1data[1]
...
#define tha tha_kv
 double tha = 25;
#define tadj_kv _thread1data[2]
#define tadj _thread[_gth]._pval[2]
#define temp temp_kv
 double temp = 23;
	...
	Prop *_prop;
{
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 4);
 	...
 	ivoc_help("help ?1 kv /home/ximing/Documents/lab_stuff/synpr/mechanisms/i686/kv.mod\n");
...
static int  rates ( _p, _ppvar, _thread, _nt, _lv ) double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt; 
 ...
 {
   double _la , _lb ;
 _la = Ra * qa * efun ( _threadargscomma_ - ( _lv - tha ) / qa ) ;
   _lb = Rb * qa * efun ( _threadargscomma_ ( _lv - tha ) / qa ) ;
   tadj = pow( q10 , ( ( celsius - temp ) / 10.0 ) ) ;
   ntau = 1.0 / tadj / ( _la + _lb ) ;
   ninf = _la / ( _la + _lb ) ;
    return 0; }
